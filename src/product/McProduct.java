...
public class McProduct {
    ...
    private final static Bugger BIGMAC
            = new Bugger(McMenu.BUGGERS.getMenuName(), "Big Mac", "참깨빵 위에 순쇠고기 패티두장 특별한 소스, 양상추, 치즈, 피클 양파까지!",
            5.2);
    ...
    private final static Bugger SHUSHU
            = new Bugger(McMenu.BUGGERS.getMenuName(), "Supreme Shrimp Burger", "탱~글한 통새우살이 가득 들어있는 슈슈버거!", 4.7);
    private final static Bugger BULGOGI
            = new Bugger(McMenu.BUGGERS.getMenuName(), "Bulgogi Burger",
            "한국인의 입맛에 딱 맞는 불고기소스에 잘재운 패티와 고소한 마요네즈, 신선한 양상추의 맛있는 조합.", 2.8);

    // set
    private final static Bugger BIGMAC_SET = new Bugger(BIGMAC,"Set", 6.5);
    private final static Bugger SANGHAI_SET = new Bugger(SANGHAI,"Set", 6.5);
    private final static Bugger SHUSHU_SET = new Bugger(SHUSHU, "Set", 6.0);
    private final static Bugger BULGOGI_SET = new Bugger(BULGOGI, "Set", 4.5);
