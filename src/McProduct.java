...
public class McProduct {
    ...
    private final static Bugger SANGHAI
            = new Bugger(McMenu.BUGGERS.getMenuName(),"Shanghai Burger",  "겉은 바삭, 속은 부드러운 치킨 패티의 매콤함으로 입맛도 기분도 화끈하게!", 6.0);
    ...
    private final static Bugger BULGOGI
            = new Bugger(McMenu.BUGGERS.getMenuName(),"Bulgogi Burger",  "한국인의 입맛에 딱 맞는 불고기소스에 잘재운 패티와 고소한 마요네즈, 신선한 양상추의 맛있는 조합.", 3.6);
    ...
    private final static Side CHEESE_STICK
                = new Side(McMenu.SIDES.getMenuName(),"Cheese Sticks 2pcs",  "속이 꽉 찬 황금빛 바삭함! 자연 모짜렐라 치즈로 빈틈없이 고소한 맥도날드 치즈스틱!", 3.3);
    ...
    private final static Beverage COKE_ZERO
            = new Beverage(McMenu.BEVERAGES.getMenuName(),"Coca-Cola Zero",  "상쾌한 맛은 살리면서 웰빙 트렌드에 맞춰 설탕과 칼로리를 제로로 줄인 코카-콜라 제로", 2.4);
